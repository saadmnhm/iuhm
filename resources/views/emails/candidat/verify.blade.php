...
<title>{{ $isAr ? 'تأكيد البريد الإلكتروني' : 'Vérification de votre email' }}</title>
</head>
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#648454 0%,#8baf74 100%);padding:40px 48px;text-align:center;">
            ...
            <h1 style="margin:0;color:#ffffff;font-size:26px;font-weight:700;letter-spacing:-0.5px;">
              {{ $isAr ? 'تأكيد البريد الإلكتروني' : 'Vérifiez votre email' }}
            </h1>
        ...
        <tr>
          <td style="padding:48px 48px 32px;">
            ...
            <p style="margin:0 0 24px;font-size:15px;color:#4a5568;line-height:1.7;">
              @if($isAr)
                شكراً لتسجيلك في منصة <strong>IUHM</strong>. أنت على بُعد خطوة واحدة من الوصول الكامل إلى حسابك.
              @else
                Merci de vous être inscrit sur la plateforme <strong>IUHM</strong>. Vous êtes à une étape de l'accès complet à votre espace.
              @endif
            </p>

            {{-- Info box --}}
            <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:16px 20px;margin-bottom:32px;">
              <p style="margin:0;font-size:14px;color:#166534;">
                <span style="font-size:16px;">🔒</span>
                {{ $isAr ? 'هذا الرابط صالح لمدة ' . $expiresIn . ' دقيقة فقط.' : 'Ce lien est valide pendant ' . $expiresIn . ' minutes uniquement.' }}
              </p>
            </div>
            ...
            <div style="text-align:center;margin-bottom:32px;">
              <a href="{{ $url }}"
                ...
                {{ $isAr ? '✅ تأكيد البريد الإلكتروني' : '✅ Vérifier mon adresse email' }}
              </a>
            </div>
            ...
            <p style="margin:24px 0 0;font-size:13px;color:#a0aec0;">
              {{ $isAr ? 'إذا لم تُنشئ حساباً، يمكنك تجاهل هذا البريد.' : 'Si vous n\'avez pas créé ce compte, ignorez cet email.' }}
            </p>
          </td>
        </tr>
