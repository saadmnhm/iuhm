...
<div class="min-h-screen ">
    ...
    >
            ...
            <div class="px-6 py-6 lg:px-8 lg:py-8">
                <div x-show="page === 'settings'" x-cloak class="space-y-6">
                    <div class="grid gap-6 lg:grid-cols-2">
                        <div class="rounded-3xl border border-slate-100 bg-slate-50/60 p-5 shadow-sm">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        class="w-full rounded-[18px] border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition placeholder:text-slate-400 focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10"
                                    >
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        class="w-full resize-none rounded-[18px] border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition placeholder:text-slate-400 focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10"
                                    ></textarea>
                                </div>
                            </div>
                        </div>

                        <div class="rounded-3xl border border-slate-100 bg-slate-50/60 p-5 shadow-sm" dir="rtl">
                            <div class="mb-5 flex items-center justify-between">
                        ...
                        <div class="flex items-center justify-between gap-4">
                            <div class="flex items-center gap-3">
                                <div class="flex h-11 w-11 items-center justify-center rounded-full bg-emerald-600 text-white shadow-lg shadow-emerald-600/20">
                                    <i class="ri-lightning-charge-line text-lg"></i>
                                </div>
                                <div>
                ...
                <div x-show="page === 'builder'" x-cloak class="space-y-6">
                    ...
                    @if($activeStep)
                        <div class="rounded-[28px] border border-slate-100 bg-white p-5 shadow-sm lg:p-6">
                            ...
                            <div x-show="editorTab === 'question'" x-cloak class="mt-5 space-y-4">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="text-sm font-black text-slate-900">Ajouter une étape</p>
                                        <p class="text-xs text-slate-500">Gérez les questions de l’étape active.</p>
                                    </div>
                                    <button
                                        type="button"
                                        wire:click="openFieldModal({{ $activeStep->id }})"
                                        class="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white px-4 py-2 text-sm font-bold text-emerald-700 transition hover:bg-emerald-50"
                                    >
                                        <span class="flex h-5 w-5 items-center justify-center rounded-full border border-emerald-500 text-emerald-600">+</span>
                                ...
                                <div class="space-y-3">
                                    @forelse($activeStep->fields ?? [] as $field)
                                        <div class="group rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4 transition hover:border-emerald-200 hover:bg-emerald-50/40">
                                            <div class="flex flex-wrap items-center justify-between gap-3">
                                                ...
                                                <div class="flex items-center gap-1 opacity-100 transition lg:opacity-0 lg:group-hover:opacity-100">
                                                    <button type="button" wire:click="openFieldModal({{ $activeStep->id }}, {{ $field->id }})" class="rounded-full p-2 text-[#0f1d57] transition hover:bg-white"><i class="ri-edit-line"></i></button>
                                    ...
                                    @empty
                                        <div class="rounded-[22px] border border-dashed border-slate-200 bg-slate-50 px-5 py-8 text-center">
                                            ...
                                            <p class="mt-4 text-sm font-bold text-slate-700">Cette étape est vide</p>
                                            <p class="mt-1 text-sm text-slate-500">Ajoutez des questions ou passez au tableau.</p>
                                        </div>
                                    @endforelse
                                </div>
                            </div>
                            ...
                            <div x-show="editorTab === 'table'" x-cloak class="mt-5 space-y-5">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="text-sm font-black text-slate-900">Ajouter une étape</p>
                                        <p class="text-xs text-slate-500">Créez des tableaux statiques ou dynamiques.</p>
                                    </div>
                                    <button
                                        type="button"
                                        wire:click="openTableModal({{ $activeStep->id }})"
                                        class="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white px-4 py-2 text-sm font-bold text-emerald-700 transition hover:bg-emerald-50"
                                    >
                                        <span class="flex h-5 w-5 items-center justify-center rounded-full border border-emerald-500 text-emerald-600">+</span>
                                        Ajouter un tableau
                                ...
                                <div class="rounded-3xl border border-slate-100 bg-slate-50 p-4">
                                    <div class="mb-4 text-xs font-black uppercase tracking-[0.16em] text-slate-400">Mode de fonctionnement</div>
                                    <div class="grid gap-3 md:grid-cols-2">
                                        <button
                                            type="button"
                                            @click="tableHasDynamicRows = false"
                                        ...
                                        >
                                            <div class="flex items-center gap-3">
                                                <div class="flex h-11 w-11 items-center justify-center rounded-xl"
                                                    :class="tableHasDynamicRows === false ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'">
                                        ...
                                        <button
                                            type="button"
                                            @click="tableHasDynamicRows = true"
                                            class="rounded-[20px] border p-4 text-left shadow-sm transition"
                                        ...
                                        >
                                            <div class="flex items-center gap-3">
                                                <div class="flex h-11 w-11 items-center justify-center rounded-xl"
                                                    :class="tableHasDynamicRows === true ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'">
                                ...
                                <div class="space-y-4">
                                    @forelse($activeStep->tables ?? [] as $table)
                                        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
                                            <div class="flex flex-wrap items-center justify-between gap-3">
                                                <div>
                                                    <div class="text-base font-black text-slate-900">{{ $table->title }}</div>
                                                    <div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                                                        <span class="rounded-full bg-slate-100 px-2.5 py-1 font-semibold">{{ $table->table_key }}</span>
                                                        @if($table->has_dynamic_rows)
                                                            <span class="rounded-full bg-emerald-100 px-2.5 py-1 font-semibold text-emerald-700">Lignes dynamiques</span>
                                                        @else
                                                            <span class="rounded-full bg-sky-100 px-2.5 py-1 font-semibold text-sky-700">Lignes fixes</span>
                                                        @endif
                                                        @if($table->has_total_row)
                                                            <span class="rounded-full bg-amber-100 px-2.5 py-1 font-semibold text-amber-700">Avec total</span>
                                                        @endif
                                                    </div>
                                                </div>

                                                <div class="flex items-center gap-1">
                                                    <button type="button" wire:click="openTableModal({{ $activeStep->id }}, {{ $table->id }})" class="rounded-full p-2 text-[#0f1d57] transition hover:bg-slate-50"><i class="ri-settings-3-line"></i></button>
                                                    <button type="button" wire:click="deleteTable({{ $table->id }})" wire:confirm="Supprimer ce tableau ?" class="rounded-full p-2 text-rose-600 transition hover:bg-slate-50"><i class="ri-delete-bin-line"></i></button>
                                                </div>
                                            </div>

                                            @if($table->columns->isNotEmpty())
                                                <div class="mt-4 overflow-x-auto rounded-[18px] border border-slate-100 bg-slate-50">
                                                    <table class="min-w-full text-sm">
                                                        <thead class="bg-white">
                                                            <tr>
                                                                @if(!$table->has_dynamic_rows)
                                                                    <th class="px-4 py-3 text-left text-xs font-black uppercase tracking-[0.14em] text-slate-500">Ligne</th>
                                                                @endif
                                                                @foreach($table->columns->sortBy('sort_order') as $col)
                                                                    <th class="px-4 py-3 text-left text-xs font-black uppercase tracking-[0.14em] text-slate-500">
                                                                        <div class="flex items-center justify-between gap-2">
                                                                            <div>
                                                                                <span class="block text-sm font-bold text-[#0f1d57]">{{ $col->header }}</span>
                                                                                <span class="text-[11px] font-normal lowercase text-slate-400">({{ $col->input_type }})</span>
                                                                            </div>
                                                                            <div class="flex items-center gap-1">
                                                                                <button type="button" wire:click="openColumnModal({{ $table->id }}, {{ $col->id }})" class="rounded-full p-1.5 text-slate-400 transition hover:bg-white hover:text-[#0f1d57]"><i class="ri-edit-line text-xs"></i></button>
                                                                                <button type="button" wire:click="deleteColumn({{ $col->id }})" wire:confirm="Supprimer cette colonne ?" class="rounded-full p-1.5 text-slate-400 transition hover:bg-white hover:text-rose-600"><i class="ri-close-line text-xs"></i></button>
                                                                            </div>
                                                                        </div>
                                                                    </th>
                                                                @endforeach
                                                                        ...
                                                                        <td class="px-4 py-3 font-medium text-slate-700">
                                                                            <div class="flex items-center justify-between gap-3">
                                                                                ...
                                                                                <div class="flex items-center gap-1 opacity-0 transition group-hover:opacity-100">
                                                                                    <button type="button" wire:click="openRowModal({{ $table->id }}, {{ $row->id }})" class="rounded-full p-1.5 text-[#0f1d57] transition hover:bg-slate-50"><i class="ri-edit-line text-xs"></i></button>
                                                                                    <button type="button" wire:click="deleteRow({{ $row->id }})" wire:confirm="Supprimer cette ligne ?" class="rounded-full p-1.5 text-rose-600 transition hover:bg-slate-50"><i class="ri-delete-bin-line text-xs"></i></button>
                                                                                </div>
                                                                            </div>
                                                                        </td>
                                                                        @foreach($table->columns->sortBy('sort_order') as $col)
                                                                            <td class="px-4 py-3 text-slate-400">{{ $col->input_type }}</td>
                                                                        @endforeach
                                                                    </tr>
                                                                @endforeach
                                                            @else
                                                                <tr>
                                                                    <td colspan="{{ $table->columns->count() + 1 }}" class="px-4 py-8 text-center text-sm text-slate-400">Aucune ligne fixe</td>
                                                                </tr>
                                                            @endif
                                                        </tbody>
                                                    </table>
                                            ...
                                            @else
                                                <div class="mt-4 rounded-[18px] border border-dashed border-slate-200 bg-slate-50 px-5 py-8 text-center text-sm text-slate-400">Aucune colonne définie</div>
                                            @endif
                                        </div>
                                    @empty
                                        <div class="rounded-[22px] border border-dashed border-slate-200 bg-slate-50 px-5 py-10 text-center">
                                            <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-emerald-100 text-emerald-700">
                                                <i class="ri-table-line text-xl"></i>
                                            </div>
                                            <p class="mt-4 text-sm font-bold text-slate-700">Aucun tableau pour cette étape</p>
                                            <p class="mt-1 text-sm text-slate-500">Ajoutez un tableau pour commencer.</p>
                                        </div>
                                    @endforelse
                                </div>
                            </div>
        ...
        {{-- Field Modal --}}
        <div x-data="{ open: @entangle('showFieldModal').live }" x-cloak>
            <div x-show="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/55 px-4 py-6" x-transition>
                <div class="absolute inset-0" @click="$wire.showFieldModal = false"></div>
                <div class="relative z-10 w-full max-w-3xl overflow-hidden rounded-4xl bg-white shadow-[0_30px_90px_rgba(0,0,0,0.28)]">
                    <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4 lg:px-7">
                        <div>
                            <h3 class="text-lg font-black text-[#0f1d57]">{{ $fieldForm['id'] ? 'Modifier la question' : 'Ajouter une question' }}</h3>
                            <p class="text-sm text-slate-500">Définissez le texte, le type et les options de la question</p>
                        </div>
                        <button type="button" wire:click="$set('showFieldModal', false)" class="rounded-full p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"><i class="ri-close-line text-xl"></i></button>
                    </div>
                    ...
                    <div class="grid gap-5 px-6 py-6 lg:grid-cols-2 lg:px-7">
                        <div>
                            ...
                            <input type="text" wire:model.blur="fieldForm.label" class="w-full rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10" placeholder="Entrez votre question ici...">
                        ...
                        <div dir="rtl">
                            ...
                            <input type="text" wire:model.blur="fieldForm.label_ar" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10" placeholder="أدخل سؤالك هنا...">
                        </div>

                        <div>
                            <label class="mb-2 block text-sm font-bold text-slate-700">Type de champ</label>
                            <select wire:model.live="fieldForm.type" class="w-full rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10">
                        ...
                        <div>
                            <label class="mb-2 block text-sm font-bold text-slate-700">Type de champ</label>
                            <input type="text" wire:model.blur="fieldForm.placeholder" class="w-full rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10" placeholder="Ex: Jean Dupont">
                        </div>

                        <div class="lg:col-span-2">
                            <label class="mb-2 block text-sm font-bold text-slate-700">Bulle d'aide (Français)</label>
                            <textarea wire:model.blur="fieldForm.help_text" rows="4" class="w-full resize-none rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10" placeholder="Information complémentaire..."></textarea>
                        </div>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4 lg:px-7">
                        <button type="button" wire:click="$set('showFieldModal', false)" class="rounded-full border border-slate-200 bg-white px-5 py-2.5 text-sm font-bold text-slate-600">Annuler</button>
        ...
        {{-- Table Modal --}}
        <div x-data="{ open: @entangle('showTableModal').live }" x-cloak>
            <div x-show="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/55 px-4 py-6" x-transition>
                <div class="absolute inset-0" @click="$wire.showTableModal = false"></div>
                <div class="relative z-10 w-full max-w-4xl overflow-hidden rounded-4xl bg-white shadow-[0_30px_90px_rgba(0,0,0,0.28)]">
                    <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4 lg:px-7">
                        ...
                        <button type="button" wire:click="$set('showTableModal', false)" class="rounded-full p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"><i class="ri-close-line text-xl"></i></button>
                    </div>

                    <div class="grid gap-5 px-6 py-6 lg:grid-cols-2 lg:px-7">
                        <div>
                            <label class="mb-2 block text-sm font-bold text-slate-700">Titre</label>
                            <input type="text" wire:model.blur="tableForm.title" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10" placeholder="Type de Service">
                        ...
                        <div dir="rtl">
                            ...
                            <input type="text" wire:model.blur="tableForm.title_ar" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10" placeholder="نوع الخدمة">
                        </div>

                        <div>
                            <label class="mb-2 block text-sm font-bold text-slate-700">Clé du tableau</label>
                            <input type="text" wire:model.blur="tableForm.table_key" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10" placeholder="table_key">
                        ...
                        <div>
                            ...
                            <input type="number" wire:model.blur="tableForm.min_rows" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10">
                        </div>
                        <div>
                            <label class="mb-2 block text-sm font-bold text-slate-700">Nombre maximum de lignes</label>
                            <input type="number" wire:model.blur="tableForm.max_rows" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10">
                        </div>
                    </div>

                    <div class="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4 lg:px-7">
                        <button type="button" wire:click="$set('showTableModal', false)" class="rounded-full border border-slate-200 bg-white px-5 py-2.5 text-sm font-bold text-slate-600">Annuler</button>
        ...
        {{-- Column Modal --}}
        <div x-data="{ open: @entangle('showColumnModal').live }" x-cloak>
            <div x-show="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/55 px-4 py-6" x-transition>
                <div class="absolute inset-0" @click="$wire.showColumnModal = false"></div>
                <div class="relative z-10 w-full max-w-4xl overflow-hidden rounded-4xl bg-white shadow-[0_30px_90px_rgba(0,0,0,0.28)]">
                    <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4 lg:px-7">
                        <div>
                        ...
                        <button type="button" wire:click="$set('showColumnModal', false)" class="rounded-full p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"><i class="ri-close-line text-xl"></i></button>
                    </div>

                    <div class="grid gap-5 px-6 py-6 lg:grid-cols-2 lg:px-7">
                        <div>
                            <label class="mb-2 block text-sm font-bold text-slate-700">Titre</label>
                            <input type="text" wire:model.blur="columnForm.header" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10" placeholder="Type de Service">
                        </div>
                        <div dir="rtl">
                            <label class="mb-2 block text-sm font-bold text-slate-700">العنوان</label>
                            <input type="text" wire:model.blur="columnForm.header_ar" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10" placeholder="نوع الخدمة">
                        </div>

                        <div>
                            <label class="mb-2 block text-sm font-bold text-slate-700">Clé de colonne</label>
                            <input type="text" wire:model.blur="columnForm.column_key" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10" placeholder="column_key">
                        </div>
                        <div>
                            <label class="mb-2 block text-sm font-bold text-slate-700">Type</label>
                            <select wire:model.live="columnForm.input_type" class="w-full rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10">
                                <option value="text">Texte</option>
                                <option value="number">Nombre</option>
                                <option value="checkbox">Case à cocher</option>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4 lg:px-7">
                        <button type="button" wire:click="$set('showColumnModal', false)" class="rounded-full border border-slate-200 bg-white px-5 py-2.5 text-sm font-bold text-slate-600">Annuler</button>
        ...
        {{-- Row Modal --}}
        <div x-data="{ open: @entangle('showRowModal').live }" x-cloak>
            <div x-show="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/55 px-4 py-6" x-transition>
                <div class="absolute inset-0" @click="$wire.showRowModal = false"></div>
                <div class="relative z-10 w-full max-w-2xl overflow-hidden rounded-4xl bg-white shadow-[0_30px_90px_rgba(0,0,0,0.28)]">
                    <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4 lg:px-7">
                        ...
                        <button type="button" wire:click="$set('showRowModal', false)" class="rounded-full p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"><i class="ri-close-line text-xl"></i></button>
                    </div>

                    <div class="grid gap-5 px-6 py-6 lg:grid-cols-2 lg:px-7">
                        <div>
                            <label class="mb-2 block text-sm font-bold text-slate-700">Libellé (Français)</label>
                            <input type="text" wire:model.blur="rowForm.label" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10" placeholder="Saisir le titre de la ligne">
                        </div>
                        <div dir="rtl">
                            <label class="mb-2 block text-sm font-bold text-slate-700">التسمية (العربية)</label>
                            <input type="text" wire:model.blur="rowForm.label_ar" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[#0f1d57] focus:ring-4 focus:ring-[#0f1d57]/10" placeholder="أدخل عنوان السطر">
                        </div>
                    </div>

                    <div class="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4 lg:px-7">
                        <button type="button" wire:click="$set('showRowModal', false)" class="rounded-full border border-slate-200 bg-white px-5 py-2.5 text-sm font-bold text-slate-600">Annuler</button>
