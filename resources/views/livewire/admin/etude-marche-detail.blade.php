<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-indigo-50">
            <h2 class="text-lg font-bold text-gray-800">Étude de Marché - Détails</h2>
        </div>
        <div class="p-6">
            @if($etudeMarche)
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="font-medium">{{ $etudeMarche->candidat->nom ?? '' }} {{ $etudeMarche->candidat->prenom ?? '' }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500">Date de création</p>
                    <p class="font-medium">{{ $etudeMarche->created_at?->format('d/m/Y H:i') }}</p>
                </div>
            </div>
